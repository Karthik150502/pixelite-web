...
const renderSquares = (data: typeof squareData) =>
    data.map((sq) => (
        <motion.div
            key={sq.id}
            layout
...
const generateSquares = () => renderSquares(shuffle(squareData));

const ShuffleGrid = () => {
    const timeoutRef = useRef<NodeJS.Timeout | null>(null);
    // Deterministic (unshuffled) order on first render so the server and
    // client HTML match; the actual shuffle only runs client-side after
    ...
    return (
        <div className="grid grid-cols-4 grid-rows-4 h-112.5 gap-1">
            {squares.map((sq) => sq)}
        </div>
    );
};
